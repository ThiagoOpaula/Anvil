...
mod tests {
    ...
    fn no_backups_in_empty_dir() {
        let dir = std::env::temp_dir().join("mod-updater-test-no-backups");
    ...
    fn finds_latest_backup() {
        let dir = std::env::temp_dir().join("mod-updater-test-backups");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
    ...
    fn create_backup_dir_creates_directory() {
        let dir = std::env::temp_dir().join("mod-updater-test-create-backup");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
